...
//float now_s() __attribute__ ((no_instrument_function));
//float now_s() {
...
//inline unsigned long long rdtsc() __attribute__ ((no_instrument_function));
...
uint64_t rdtsc() __attribute__ ((no_instrument_function));
uint64_t rdtsc() {
	uint32_t lo, hi;
...
__cyg_profile_func_enter (void *this_fn, void *call_site)
{
        if(cyg_profile_enabled)
                if (logfile || openlogfile(cyg_profile_filename))
                        fprintf(logfile, "call(%d) %p -> %p %" PRIu64 "\n", level++,
                                call_site, this_fn, rdtsc());
}
...
__cyg_profile_func_exit (void *this_fn, void *call_site)
{
        if(cyg_profile_enabled)
                if (logfile || openlogfile(cyg_profile_filename))
                        fprintf(logfile, "return(%d) %p <- %p %" PRIu64 "\n", level--,
                                call_site, this_fn, rdtsc());
}
