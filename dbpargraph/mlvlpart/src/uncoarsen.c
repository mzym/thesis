/* 
Graph partitioning with Parallel DBMS: uncoarsening phase.
(c) Mikhail Zymbler 
*/
...
int main(int argc, char **argv) {
	 ...
	 */
	if (argc != 6) {
		fprintf(stderr, "Need five arguments: conninfo, fine tablename, matching tablename, coarse partition tablename, fine partition tablename\n");
		exit(1);
	}
	...
	/* Send the queries! */
	sprintf(query, "select uncoarsen('%s', '%s', '%s', '%s');", tablename, matchingname, coarsename, finename);
