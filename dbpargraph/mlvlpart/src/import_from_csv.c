...
int main(int argc, char **argv) {
	...
	sprintf(query, "drop table if exists %s; create table %s (a int, b int, w float) with (fragattr = a);", tablename, tablename);
	...
	while (feof(stdin) == 0) {
		fscanf(stdin, "%d,%d,%f\n", &i, &j, &w);
		sprintf(query, "insert into %s values (%d, %d, %0.2f);", tablename, i, j, w);
		fprintf(stderr, "Sending query: %s\n", query);
